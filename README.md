...
# Data set
The set used is a Store Data set that has sales transactional fake data. The main reason to use this set is because the possible visualization for this set are known, the author has personal experience working with it and offer a good contrast between what can be build with it in comparison with other tools.
...
One of the most important features is that allows the user to compare the sales to date during the current year with the sales goal. It has a dynamic color configuration that changes the color of the difference when is positive to green and keep it in red until the goal is reached.

This report has some limitations. The ideal situation would be to calculate the goal and the previous year difference based on the selections of different variables as category or segment. But to do that, we would need to build a table that has this metrics and dimensions in the same row. That is totally possible to implement, but was prefered to not do it because of time margin limitations.

## Sales by state report
The purpouse of this report is to show the amount sold in a date range between the different states of the USA. You will see 2 metrics in the graph. Sales, represented by the size of the bubble and profit which is represented with the color of the bubble.
